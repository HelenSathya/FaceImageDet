...
class _MyHomePageState extends State<MyImagePicker> {
  ...
  Future<File?> cropFaceFromImage(String imagePath) async {
    ...
    final croppedImageFile = (await imageCropper.cropImage(
      ...
      //left: faceBoundingBox.left.toInt(),
      //y: faceBoundingBox.top.toInt(),
      maxWidth: faceBoundingBox.width.toInt(),
      maxHeight: faceBoundingBox.height.toInt(),
      uiSettings: [
        AndroidUiSettings(
    ...
    final CroppedFile? croppedFile = await croppedImageFile;
    final croppedFileFinal = File(croppedFile!.path);
    print('Cropped image saved to: $croppedImageFile.path');
  ...
  Future<void> _pickAndSaveImage() async {
    ...
    final File? croppedImageFile = await cropFaceFromImage(pickedImage.path);//cropFacesUsingMLModel(pickedImage.path);
    ...
    try {
      ...
      saveImageToFirestore(croppedImageFile, tenDigitNo);
      setState(() {
  ...
  Future<File?> cropFacesUsingMLModel(String imageFilePath) async {
    ...
    try {
      ...
      for (Face face in faces) {
        ...
        //num right = min(0, faceRect.right); // Limit right to image width
        //num bottom = min(0, faceRect.bottom); // Limit bottom to image height
        num width = max(0, right - left); // Ensure width is non-negative
        num height = max(0, bottom - top); // Ensure height is non-negative
        // Check if the adjusted left and top values need to be adjusted again
  ...
  Future<void> _loadImageFromScanOrDB() async {
    try {
      ...
      File croppedScannedImageFile =  cropFaceFromImage(scannedImage.path) as File; //cropFacesUsingMLModel(scannedImage.path) as File;
      print("2");
